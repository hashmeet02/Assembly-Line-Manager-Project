...
namespace sdds {
	...
	Station::Station(const std::string& rec)
	{
		...
		Utilities util;
		bool more = true;
		m_item = util.extractToken(rec, next_pos, more);
		m_SNo = stoi(util.extractToken(rec, next_pos, more));
		m_stock = stoi(util.extractToken(rec, next_pos, more));
		m_widthField = std::max(util.getFieldWidth(), m_widthField);
		m_description = util.extractToken(rec, next_pos, more);
